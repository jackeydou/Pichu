...
DEFAULT_PLUGIN_PARENT = Path.cwd() / "apps" / "pichu-client" / "resources" / "plugins" / "plugins"
DEFAULT_MARKETPLACE_PATH = (
    Path.cwd() / "apps" / "pichu-client" / "resources" / "plugins" / "marketplace.json"
)
...
def update_marketplace_json(
    ...
    new_entry = build_marketplace_entry(
        plugin_name, source_path, install_policy, auth_policy, category
    )
...
def marketplace_root(marketplace_path: Path) -> Path:
    if marketplace_path.parent.name == "plugins":
        return marketplace_path.parent.parent
    return marketplace_path.parent


def marketplace_source_path(marketplace_path: Path, plugin_root: Path) -> str:
    relative = os.path.relpath(plugin_root, marketplace_root(marketplace_path))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(defaults to <cwd>/apps/pichu-client/resources/plugins/plugins)."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Create or update "
            "<cwd>/apps/pichu-client/resources/plugins/marketplace.json."
        ),
    )
    parser.add_argument(
        "--marketplace-path",
        ...
        help=(
            ...
            "(defaults to <cwd>/apps/pichu-client/resources/plugins/marketplace.json)."
        ),
    )
    parser.add_argument(
        "--install-policy",
...
def main() -> None:
    ...
    if args.with_hooks:
        create_stub_file(
            plugin_root / "com.pichu.app" / "hooks" / "hooks.json",
            {"hooks": {}},
            args.force,
        )
    ...
    if args.with_marketplace:
        ...
        update_marketplace_json(
            marketplace_path,
            plugin_name,
            marketplace_source_path(marketplace_path, plugin_root),
            args.install_policy,
            args.auth_policy,
            args.category,
            args.force,
