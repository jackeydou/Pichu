...
- **Short option sets and single-select filters use `Select`.** Do not create native `<select>` or `NativeSelect` controls unless the user explicitly asks for no-JS/native fallback behavior.
...
- **Use `Select`** instead of raw `<select>` for app UI.
...
**Newest styles:** when a newer style such as Sera or Rhea is not accepted as a named preset by the installed CLI, use its opaque preset code or full shadcn/create URL from ui.shadcn.com. For example, the Rhea create preset is `b27GcrRo`.
