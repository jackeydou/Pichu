...
> **IMPORTANT:** Prefer the flags documented below. If a needed flag is not listed, verify it with `npx shadcn@latest <command> --help` before use. The CLI auto-detects the package manager from the project's lockfile; there is no `--package-manager` flag.
...
> Newer styles may appear on shadcn/create before the CLI accepts them as named presets. In that case, use the preset code or full create URL. Rhea's current create preset code is `b27GcrRo`.
